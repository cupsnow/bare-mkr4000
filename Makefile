...
#
-include site.mk
PROJDIR:=$(abspath $(dir $(wildcard projdir/proj.mk ./proj.mk)))
...
CMSIS_PATH?=/home/joelai/02_dev/test26/package/CMSIS_5

BOSSAC=$(PROJDIR)/bossa/bin/bossac --offset=0x2000 --verify --info \
  --port=$(notdir $(wildcard /dev/ttyACM0 /dev/ttyACM1))
ARMGCC=/home/joelai/07_sw/pkg/gcc-arm-none-eabi-8-2019-q3-update/bin/arm-none-eabi-gcc
...
#------------------------------------
#
test3_APP=$(patsubst $(PWD)/%,%,$(BUILDDIR)/test3)
...
$(eval $(call BUILD1,test3,$(patsubst $(PWD)/%,%, \
...
#------------------------------------
#
test2_APP=$(patsubst $(PWD)/%,%,$(BUILDDIR)/test2)
...
$(eval $(call BUILD1,test2,$(patsubst $(PWD)/%,%, \
...
#------------------------------------
#
test1_APP=$(patsubst $(PWD)/%,%,$(BUILDDIR)/test1)
test1_CPPFLAGS+=-ffunction-sections -fdata-sections \
  -g -I$(PROJDIR)/moss/inc -I$(PROJDIR)/moss/openbsd
test1 test1%: CROSS_COMPILE= 
$(eval $(call BUILD1,test1,$(patsubst $(PWD)/%,%, \
  $(PROJDIR)/moss/moss.c $(PROJDIR)/moss/moss_pc.c \
  $(PROJDIR)/test/test1.cpp)))

#------------------------------------
#
bootloader_APP=$(patsubst $(PWD)/%,%,$(BUILDDIR)/bootloader)
...
$(eval $(call BUILD1,bootloader,$(patsubst $(PWD)/%,%, \
...
#------------------------------------
#
$(eval $(call BUILD1_COMPILE,BUILD1))
